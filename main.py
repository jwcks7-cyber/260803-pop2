...
#      이 기능은 '65세 이상 비율' 지표를 볼 때만 동작합니다.
# -----------------------------------------------------------------
...
# -----------------------------------------------------------------
# 5-1. 강조 버튼이 켜져 있고, 현재 지표가 '65세 이상 비율'일 때만
#      성남시의 65세 이상 인구(고령인구)를 구합니다.
#      데이터에 따라 시군구 이름이 '성남시 분당구'처럼 붙어 있기도 하고,
#      '분당구'처럼 구 이름만 단독으로 들어있기도 해서 두 경우를 모두 찾습니다.
# -----------------------------------------------------------------
성남시_고령인구 = None
if st.session_state.성남시_강조:
    if not 설정["성남시비교가능"]:
        st.info("🟣 이 강조 기능은 '65세 이상 비율' 지표를 선택했을 때만 사용할 수 있습니다.")
    else:
        이름 = sigungu_df["시군구"].astype(str).str.strip()
        성남시_조건 = 이름.str.contains("성남") | (
            (sigungu_df["시도"].astype(str).str.strip() == "경기도")
            & 이름.isin(["분당구", "수정구", "중원구"])
        )
        성남시_행 = sigungu_df[성남시_조건]

        if len(성남시_행) > 0:
            성남시_고령인구 = 성남시_행["고령인구"].sum()
            st.success(
                f"✅ {선택연도}년 '성남시'로 인식한 지역: "
                f"{', '.join(성남시_행['시군구'].tolist())} "
                f"(65세 이상 인구 합계 {성남시_고령인구:,.0f}명)"
            )
        else:
...
# 강조 버튼이 켜져 있으면(그리고 지표가 65세 이상 비율이면), 성남시보다
# 65세 이상 인구가 많은 지역은 원래 색 구간을 덮어쓰고 보라색으로 표시합니다.
...
if 성남시_고령인구 is not None:
    강조된_개수 = int((map_df["구간"] == "성남시보다 고령인구 많음").sum())
    st.caption(
        f"🟣 보라색 = {선택연도}년 성남시 65세 이상 인구({성남시_고령인구:,.0f}명)보다 "
        f"65세 이상 인구가 많은 지역 (현재 {강조된_개수}개 지역 해당)"
    )
